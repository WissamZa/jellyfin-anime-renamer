...
class TMDBFetcher(EpisodeFetcher):
    ...
    def fetch(self) -> Optional[dict[int, EpisodeInfo]]:
        ...
        def fetch_season_en(s: dict) -> tuple[int, list]:
            ...
            if not data:
                log.warning("TMDB: failed to fetch season %d (English) — will retry once", sn)
        ...
        def enrich_with_ja_titles(sn: int, eps: list) -> list:
            ...
            if not data_ja:
                return eps
            ja_map = {e["episode_number"]: e.get("name", "") for e in data_ja.get("episodes", [])}
            for ep in eps:
...
class EpisodeNumberParser:
    ...
    SEASON_EP_PATTERNS = [
        ...
        re.compile(r"\b(\d{1,2})(?:st|nd|rd|th)\s+Season\s*[-–_]\s*(\d{1,4})\b", re.IGNORECASE),
    ]
...
class SeriesCache:
    ...
    def load(self) -> Optional[dict]:
        ...
        if "series_name" not in data:
            ...
            return None
        if "tmdb_series_id" not in data and "anilist_id" not in data and "kitsu_id" not in data:
...
class AnimeRenamer:
        ...
        for path in files:
            ...
            if season_num is not None and ep_num is not None:
                ...
                if abs_num not in episode_map:
                    ...
                    results.append(
                        RenameResult(path.name, "", EpisodeInfo(abs_num, 0, 0, ""), skipped=True)
                    )
                    continue
    ...
    def _handle_file(
        ...
        season_tag = "SP" if info.is_special else f"S{info.season:02d}E{info.episode:02d}"
        log.info(
            "%s  |  %s  →  %s",
            season_tag,
            path.name,
