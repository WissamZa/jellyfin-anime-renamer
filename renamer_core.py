...
# Module-level cache shared across all KitsuFetcher instances
_api_cache: dict = {}
...
class KitsuFetcher(EpisodeFetcher):
    ...
    def find_series(self, name: str) -> tuple[int, str] | None:
        ...
        if best_id is None or best_title is None or best_score < self.MIN_SIMILARITY:
            log.info(
    ...
    def run(self, dry_run: bool = True) -> list[RenameResult]:
        ...
        if Provider.AniList == self._cfg.PROVIDER:
            ...
            if self._cfg.TMDB_SERIES_ID is None:
                log.error("Could not build episode map — TMDB_SERIES_ID is missing.")
                return []
            fetcher = TMDBFetcher(
